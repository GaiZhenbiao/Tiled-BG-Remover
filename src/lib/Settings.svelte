...
<div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" on:click={() => dispatch('close')}>
  <div class="bg-gray-800 p-6 rounded-lg shadow-xl w-96 border border-gray-700" on:click|stopPropagation>
    ...
    <div class="mb-4">
      <label for="api-key" class="block text-sm font-medium mb-1">Google AI API Key</label>
    ...
    <div class="mb-4">
      <label for="model-name" class="block text-sm font-medium mb-1">Model Name</label>
      <input id="model-name" type="text" bind:value={modelName} class="w-full bg-gray-700 border border-gray-600 rounded p-2 text-white" placeholder="gemini-1.5-pro" />
